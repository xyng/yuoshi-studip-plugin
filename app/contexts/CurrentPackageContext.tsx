...
type updatePackageFn = (data: Partial<{
    title: string,
    slug: string
...
const CurrentPackageContext = createContext<CurrentPackageContextInterface | null>(null)
...
export const CurrentPackageContextProvider: React.FC<{
    ...
    const updateCurrentPackage = useCallback(async ({ title, slug }, reload = false) => {
        ...
        const newData = (await currentPackageData.save()).getModel() as Package

        // update item in list if we've got it from there
        if (currentPackageFromList) {
            return updatePackage(newData.getApiId() as string, newData, reload)
        }
    ...
    }, [currentPackageData, currentPackageFromList, mutate, reloadPackages, updatePackage])
    ...
    return <CurrentPackageContext.Provider value={ctx}>
        {children}
